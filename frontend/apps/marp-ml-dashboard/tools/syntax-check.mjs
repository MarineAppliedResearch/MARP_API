 ...
 *
 * There is no build step here, which is a feature — but it means nothing reads
 * the source before a browser does. In the Mosaic Reviewer a stray quote took a
 * whole page down and the only symptom was a suite that hung for sixty seconds
 * waiting for results that were never going to arrive. This runs in about a
 * second and says which file and line.
 *
