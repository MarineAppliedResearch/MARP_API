 ...
 *
 * A3, answered by the human: **serving a mosaic page enqueues its missing
 * thumbnails**, so the `queued` tile the client already draws is truthful, only
 * what somebody is actually looking at is ever extracted, and the queue is
 * bounded by the working set rather than by 440,000 rows.
 *
 * `ON CONFLICT DO NOTHING` is what keeps that safe: an observation that already
 * failed permanently keeps its row and is **not** re-enqueued, which is the whole
 * reason permanence is recorded. A page of hopeless legacy rows would otherwise
 * ask Jellyfin again on every page view, for ever.
 *
 * @async
 ...
 * and never re-queued by the retry route, so the *Ask again* button the page
 * invites the reviewer to press stops being a way to hammer Jellyfin.
 *
 ...
 *
 * The rows return to being **simply absent**, which under A3 means the next page
 * view re-enqueues them. So nothing is lost, and no fourth state had to be
