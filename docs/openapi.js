            ...
            components: {
                ...
                schemas: {
                    ...
                    MosaicRow: {
                        type: 'object',
                        description:
                            'One mosaic tile: what the tile renders, plus the `version` the commit routes require back. `processor_name` and `lineId` remain absent. Current decision authors are represented only by compact initials derived from their existing usernames; full identity and activity reporting remain outside this observations:read route. `first_framenum` is here because it is free from the same lateral as `keyframe_count` and addressing a thumbnail needs it.',
                        properties: {
                            ...
                            review_note: { type: 'string', nullable: true, maxLength: 1000, example: null, description: 'Optional plain-text note on the current scientific decision.' },
                            review_reviewer_id: { type: 'integer', nullable: true, example: 42, description: 'users.user_id of whoever made the current scientific decision, or null where there is none. The separate initials field is derived from the referenced user at query time.' },
                            review_reviewer_initials: { type: 'string', nullable: true, minLength: 2, maxLength: 2, example: 'IT', description: 'Two uppercase initials derived at query time from the username of the current scientific decision author. A separated username uses the first and last parts; a single-part username uses its first two characters. No initials are stored on the review row.' },
                            ...
                            training_note: { type: 'string', nullable: true, maxLength: 1000, example: null, description: 'Optional plain-text note on the current training decision.' },
                            training_reviewer_id: { type: 'integer', nullable: true, example: 42, description: 'users.user_id of whoever made the current training decision, or null where there is none. The separate initials field is derived from the referenced user at query time.' },
                            training_reviewer_initials: { type: 'string', nullable: true, minLength: 2, maxLength: 2, example: 'IT', description: 'Two uppercase initials derived at query time from the username of the current training decision author. A separated username uses the first and last parts; a single-part username uses its first two characters. No initials are stored on the review row.' },
                    ...
                    MosaicCommitRequest: {
                        ...
                        properties: {
                            ...
                            marks: {
                                type: 'array',
                                description: 'What the reviewer marked, and what each mark means. Every id must be in `observations`. The reason is optional and comes from a closed vocabulary -- the reviewer-facing list for that mode -- so an unknown value is a 400 rather than a truncated or silently dropped reason. Only an exception takes a reason: a reason says what is wrong with an observation, so a reason on an `accept` mark is a 400. A review or training mark of either kind may carry an optional plain-text note. Notes are trimmed, blank notes become null, and notes longer than 1,000 Unicode characters are refused. The delete route records neither reason nor note and refuses an `accept` mark outright because it has no accepted state.',
                                items: {
                                    type: 'object',
                                    required: ['observation_id'],
                                    properties: {
                                        ...
                                        note: { type: 'string', nullable: true, maxLength: 1000, example: 'Tentative identification; check the final keyframe.' },
                                        kind: {
                                            type: 'string',
                                            enum: ['except', 'accept'],
                                            default: 'except',
