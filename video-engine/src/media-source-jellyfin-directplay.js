 ...
 * lets fetches run at full concurrency.
 *
 * Units here are GOPs taken from the file's own sample table rather than
 * fixed-length HLS segments, so their timing is authoritative rather than
 * derived from playlist durations. Because the sample table carries every
 * sample's offset, size and timestamp, chunk assembly needs no per-unit
 * container parsing at all -- unlike the transcode path, which demuxes
 * init+media with mp4box for every segment.
 *
...
/**
 * Supplies the unit index and decoder chunks for a Direct Play stream.
 *
 * @class JellyfinDirectPlayMediaSource
 */
export class JellyfinDirectPlayMediaSource {
    /**
     * @param {Object} params
     ...
     */
    constructor({ client, itemId, fetchOptions, rawSegmentCacheBudgetBytes, indexPrefixBytes, onDebug, onError }) {
        this.client = client;
        this.itemId = itemId;
     ...
     */
    _logDebug(message) {
        const prefixed = `[media-source-jellyfin-directplay] ${message}`;
