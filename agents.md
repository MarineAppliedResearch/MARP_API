...
On that box: this account's home directory holds `~/jellyfin-src/jellyfin` (git repo, branch `master`, pushed to `git@github.com:MarineAppliedResearch/marp-jellyfin.git` as `origin` -- this is MARP's own ongoing Jellyfin fork, not just a one-off patch; `upstream` remote still points at `github.com/jellyfin/jellyfin.git` for pulling future updates. Push access uses a dedicated key at `~/.ssh/id_ed25519` on that account, registered under Isaac's personal GitHub account -- use `GIT_SSH_COMMAND='ssh -i ~/.ssh/id_ed25519'` if the default identity resolution doesn't pick it up. The repo was originally a shallow (`--depth 1`) clone, which caused a `did not receive expected object` push failure until unshallowed with `git fetch --unshallow upstream` -- it's a full clone now, so this shouldn't recur. Current fix commit: `-noaccurate_seek` wrong-segment-content bug in `EncodingHelper.cs`, see that commit message for the full writeup) and `~/jellyfin-src/jellyfin-web` (unmodified `v10.11.11` clone, not yet pushed anywhere). `~/.dotnet` has .NET 9.0.317; `~/.nvm` has Node 20.20.2. The dev instance runs on **port 8097** (never 8096 -- that's the real, live, production `jellyfin.service`; do not touch it, its config, or its systemd unit), with its own datadir at `~/jellyfin-src/data` and web client linked from `~/jellyfin-src/jellyfin-web/dist`. Its own `network.xml` had to be hand-authored before first launch since there's no `--port` CLI flag -- the runtime config class is `MediaBrowser.Common.Net.NetworkConfiguration` (`InternalHttpPort`/`PublicHttpPort`, not the differently-named fields in the migration-only `OldNetworkConfiguration` class -- easy to mix up). Admin login: `admin` / `MarpDevJellyfinRemote2026!`. Its library holds two videos copied (not symlinked) from the real production media archive at `/mnt/rov-video-new/...` into `~/jellyfin-src/media/` -- keep dev-instance media copied into its own folder, not pointed at the live archive tree.
