 ...
 * asked for* -- and is never pre-created. It reports `queued` to the client, and
 * two triggers are what make that honest rather than a promise nobody keeps:
 * `keyframes_enqueue_thumbnail_trigger` writes the row when the keyframes are
    ...
    ObservationThumbnail.init(
        {
            ...
            thumbnail_accessed_at: { type: DataTypes.DATE, allowNull: true },
            thumbnail_evicted_at: { type: DataTypes.DATE, allowNull: true },
            full_frame_status: { type: DataTypes.STRING(16), allowNull: true },
            full_frame_permanent: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
            full_frame_framenum: { type: DataTypes.INTEGER, allowNull: true },
            full_frame_subset: { type: DataTypes.STRING(64), allowNull: true },
            full_frame_box: { type: DataTypes.JSONB, allowNull: true },
            full_frame_filename: { type: DataTypes.STRING(255), allowNull: true },
            full_frame_content_type: { type: DataTypes.STRING(64), allowNull: true },
            full_frame_byte_size: { type: DataTypes.BIGINT, allowNull: true },
            full_frame_width: { type: DataTypes.INTEGER, allowNull: true },
            full_frame_height: { type: DataTypes.INTEGER, allowNull: true },
            full_frame_generation: { type: DataTypes.INTEGER, allowNull: false, defaultValue: 1 },
            full_frame_last_error: { type: DataTypes.TEXT, allowNull: true },
            full_frame_requested_at: { type: DataTypes.DATE, allowNull: true },
            full_frame_claimed_at: { type: DataTypes.DATE, allowNull: true },
            full_frame_completed_at: { type: DataTypes.DATE, allowNull: true },
            full_frame_accessed_at: { type: DataTypes.DATE, allowNull: true },
            full_frame_evicted_at: { type: DataTypes.DATE, allowNull: true },
        },
        {
            sequelize,
