 ...
 *
 * @param {Object} range - `{start_frame, end_frame}`, both inclusive.
...
async function submitJob(overrides = {}) {
    ...
    const response = await global.api
        ...
        .send({
            kind: 'inference',
            spec: specFor({ start_frame: 0, end_frame: 99 }),
...
describe('GPU job submission and leasing', () => {
    it('queues one job and leases it, in one poll and with no separate claim', async () => {
        ...
        expect(polled.body.spec.range).toEqual({ start_frame: 0, end_frame: 99 });
    ...
    it('splits a range into pieces that tile it exactly, sharing one batch_id', async () => {
        const submitted = await submitJob({
            spec: specFor({ start_frame: 0, end_frame: 249 }),
            piece_frames: 100,
        });

        expect(submitted.batch_id).toEqual(expect.any(String));
        expect(submitted.jobs).toHaveLength(3);
        expect(submitted.jobs.map((job) => job.spec.range)).toEqual([
            { start_frame: 0, end_frame: 99 },
            { start_frame: 100, end_frame: 199 },
            { start_frame: 200, end_frame: 249 },
        ]);
