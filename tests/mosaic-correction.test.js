...
describe('the mosaic species correction (#111)', () => {
    ...
    describe('what the mosaic row shows afterwards (R19)', () => {
        ...
        it('serves the new species name while comname still serves the old label', async () => {
            ...
            // Read through the full-permission fixture agent: the pages route
            // takes observations:read and these reviewers hold only write, which
            // is right for them and beside the point here.
            const res = await global.api.post('/api/v2/mosaic/observations/pages').send({
                filters: { dive: [`JEST-CORRECT-${runId}`] },
                pageSize: 45,
                pages: [1],
            });
        ...
        it('serves a null species name where the observation has no species', async () => {
            ...
            // About 4% of production rows carry no species_id. The join is outer,
            // so the row is never dropped for want of one.
            expect(row).toBeDefined();
            expect(row.species_comname).toBeNull();
            expect(row.comname).toBe(`Jest Correction ${runId}`);
        });
    });
